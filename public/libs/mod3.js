...
!(function (root, name, factory) {
  ...
  if ('object' === typeof module && module.exports)
    ...
    (module.$deps = module.$deps || {}) &&
      (module.exports = module.$deps[name] = factory.call(root));
  else if (
    'function' === typeof define &&
    define.amd &&
    'function' === typeof require &&
    'function' === typeof require.specified &&
    require.specified(name) /*&& !require.defined(name)*/
  )
    /* AMD */
    define(name, ['module'], function (module) {
      factory.moduleUri = module.uri;
      return factory.call(root);
    });
  else if (!(name in root))
    /* Browser/WebWorker/.. */
    (root[name] = factory.call(root) || 1) &&
      'function' === typeof define &&
      define.amd &&
      define(function () {
        return root[name];
  ...
  /* module factory */ function ModuleFactory__MOD3(undef) {
    ...
    function makeSuper(superklass) {
      ...
      return function $super(method, args) {
        var self = this,
          ...
          ret;
        if (1 === called[m])
          return (superklass.prototype.$super || NOP).call(self, method, args);
        called[m] = 1;
        ret = (
          'constructor' === method
            ? superklass
            : superklass.prototype[method] || NOP
        ).apply(self, args || []);
        called[m] = 0;
        return ret;
      };
    ...
    function makeClass(superklass, klass, statik) {
      ...
      var C = HAS.call(klass, 'constructor')
          ? klass.constructor
          : function () {},
        p;
      if (superklass) {
     ...
     **/
    MOD3.Matrix = MOD3.Class(
      null,
      {
        constructor: function Matrix(m11, m12, m21, m22) {
          var self = this;
          if (!(self instanceof Matrix)) return new Matrix(m11, m12, m21, m22);
        ...
        clone: function () {
          var m = this.m;
          return new MOD3.Matrix(m[0], m[1], m[2], m[3]);
        },
      },
      {
    ...
    MOD3.Vector3 = MOD3.Class(
      ...
      },
      {
        ...
        X: function (direct_or_complement) {
          return false === direct_or_complement
            ? new MOD3.Vector3(0, 1, 1)
            : new MOD3.Vector3(1, 0, 0);
        },

        Y: function (direct_or_complement) {
          return false === direct_or_complement
            ? new MOD3.Vector3(1, 0, 1)
            : new MOD3.Vector3(0, 1, 0);
        },

        Z: function (direct_or_complement) {
          return false === direct_or_complement
            ? new MOD3.Vector3(1, 1, 0)
            : new MOD3.Vector3(0, 0, 1);
        },
     ...
     **/
    MOD3.Matrix4 = MOD3.Class(
      null,
      {
        constructor: function Matrix4(
          n11,
          n12,
          n13,
          n14,
        ...
        rotate: function (rx, ry, rz, theta, reset) {
          var m = this.m,
            nCos = stdMath.cos(theta),
            nSin = stdMath.sin(theta),
            scos = 1 - nCos,
            sxy = rx * ry * scos,
            syz = ry * rz * scos,
            sxz = rx * rz * scos,
    ...
    MOD3.Matrix4.prototype.translationMatrixFromVector =
      MOD3.Matrix4.prototype.translateFromVector;
    MOD3.Matrix4.prototype.scaleMatrixFromVector =
      MOD3.Matrix4.prototype.scaleFromVector;
    MOD3.Matrix4.prototype.rotationMatrixFromVector =
      MOD3.Matrix4.prototype.rotateFromVector;
    // fast list utilities
    MOD3.List = {
      operate: function operate(x, F, F0, i0, i1, reverse) {
        ...
        if (true === reverse) {
          var i,
            ...
            q = r & 1,
            lr = l1 - r,
            Fv = q ? F(F0, x[i1], i1) : F0;
          for (i = l1 - q; i > lr; i -= 2) {
            k = i0 + i;
            Fv = F(F(Fv, x[k], k), x[k - 1], k - 1);
          }
          for (i = lr; i >= 0; i -= 16) {
            k = i0 + i;
            Fv = F(
    ...
    MOD3.MeshProxy = MOD3.Class(null, {
      ...
      analyzeGeometry: function () {
        ...
        MOD3.List.each(vertices, function (v) {
          var xyz = v.getXYZ();
          v.setRatios(
            width > 0 ? (xyz[0] - minX) / width : 0,
            height > 0 ? (xyz[1] - minY) / height : 0,
            depth > 0 ? (xyz[2] - minZ) / depth : 0
          );
        });
        return self;
      },
    ...
    MOD3.ModifierStack = MOD3.Class({
      constructor: function ModifierStack(lib3d, mesh) {
        var self = this;
        if (!(self instanceof ModifierStack))
          return new ModifierStack(lib3d, mesh);
        self.stack = [];
      ...
      dispose: function (withModifiers) {
        ...
        if (withModifiers && self.stack)
          while (self.stack.length) self.stack.pop().dispose();
    ...
    // aliases
    MOD3.ModifierStack.prototype.getMeshInfo =
      MOD3.ModifierStack.prototype.getModifiable;
...
[/DOC_MD]**/
      ...
      MOD3.Bloat = MOD3.Class(MOD3.Modifier, {
        ...
        apply: function (modifiable) {
          var self = this,
            center = self.center.xyz,
            radius = stdMath.max(0, self.radius),
            a = stdMath.max(0, self.a);

          MOD3.List.each(modifiable.vertices, function (v) {
            // get a vector towards vertex
            // change norm to norm + r * exp (-a * norm)
            var uu = MOD3.Vector3.sub(v.getXYZ(), center),
              magn = MOD3.Vector3.mod(uu);
            MOD3.Vector3.muls(
              MOD3.Vector3.norm(uu),
              magn + radius * stdMath.exp(-magn * a)
            );
...
[/DOC_MD]**/
      MOD3.Twist = MOD3.Class(MOD3.Modifier, {
        ...
        apply: function (modifiable) {
          var self = this,
            tvec = self.vector.normalizeSelf().xyz,
            angle = self.angle,
            center = self.center.xyz,
            modulo = MOD3.Vector3.mod([
              0.5 * modifiable.maxX,
              0.5 * modifiable.maxY,
              0.5 * modifiable.maxZ,
            ]),
...
[/DOC_MD]**/
      ...
      MOD3.Skew = MOD3.Class(MOD3.Modifier, {
        constructor: function Skew(force, offset, power, falloff) {
          var self = this;
          if (!(self instanceof Skew))
            return new Skew(force, offset, power, falloff);
          self.$super('constructor');
          self.name = 'Skew';
          self.constraint = MOD3.ModConstant.NONE;
        ...
        apply: function (modifiable) {
          ...
          MOD3.List.each(modifiable.vertices, function (v) {
            ...
            if (MOD3.ModConstant.LEFT === constraint && vRatio <= offset)
              return;
            if (MOD3.ModConstant.RIGHT === constraint && vRatio > offset)
              return;
      ...
      MOD3.Taper = MOD3.Class(MOD3.Modifier, {
        ...
        apply: function (modifiable) {
          ...
          MOD3.List.each(
            ...
            1 !== power
              ? function (v) {
                  var ar = MOD3.Vector3.mod(
                      MOD3.Vector3.mul(v.getRatioVector(), vec2)
                    ),
                    sc = force * stdMath.pow(ar, power);
                  v.setXYZ(
                    MOD3.Matrix4.multXYZ(
                      m.scale(
                        1 + sc * vec[0],
                        1 + sc * vec[1],
                        1 + sc * vec[2]
 ...
 [/DOC_MD]**/
      MOD3.Wheel = MOD3.Class(MOD3.Modifier, {
        constructor: function Wheel(
          ...
          var self = this;
          if (!(self instanceof Wheel))
            return new Wheel(speed, turn, roll, steerVector, rollVector);
          self.$super('constructor');
          self.name = 'Wheel';
          self.speed = speed || 0;
          self.turn = turn || 0;
        ...
        apply: function (modifiable) {
          ...
          if (turn) {
            ...
            ms = new MOD3.Matrix4().rotateFromVector(
              mt.multiplyVector(rollVector.clone()),
              roll
            );
          } else {
            ...
            mt
              ? function (v) {
                  v.setXYZ(
                    MOD3.Matrix4.multXYZ(
                      ms,
                      MOD3.Matrix4.multXYZ(mt, v.getXYZ())
                    )
                  );
                }
              : function (v) {
      ...
      MOD3.Break = MOD3.Class(MOD3.Modifier, {
        ...
        apply: function (modifiable) {
          var self = this,
            ...
            range = self.range,
            angle = self.angle,
            bv = self.vector.normalizeSelf().xyz,
            pv,
            rm;
          ...
          MOD3.List.each(modifiable.vertices, function (v) {
            var c = v.getXYZ();
            c[2] -= pv;
            if (0 <= c[2] && range.isIn(v.ratio[1]))
              MOD3.Matrix4.multXYZ(rm, c);
            c[2] += pv;
            v.setXYZ(c);
          });
          return self;
...
[/DOC_MD]**/
      ...
      MOD3.Noise = MOD3.Class(MOD3.Modifier, {
        constructor: function Noise(force) {
          ...
          self.end = 1;
          self.axes =
            MOD3.ModConstant.X | MOD3.ModConstant.Y | MOD3.ModConstant.Z;
        },
...
[/DOC_MD]**/
      MOD3.DisplaceMap = MOD3.Class(MOD3.Modifier, {
        constructor: function DisplaceMap(bmp, force, offset) {
          var self = this;
          if (!(self instanceof DisplaceMap))
            return new DisplaceMap(bmp, force, offset);
          self.$super('constructor');
        ...
        apply: function (modifiable) {
          ...
          MOD3.List.each(modifiable.vertices, function (v) {
            ...
            v.setXYZ([
              xyz[0] +
                (axes & MOD3.ModConstant.X
                  ? ((bmp[uv] & 0xff) - offset) * force
                  : 0),
              xyz[1] +
                (axes & MOD3.ModConstant.Y
                  ? ((bmp[uv + 1] & 0xff) - offset) * force
                  : 0),
              xyz[2] +
                (axes & MOD3.ModConstant.Z
                  ? ((bmp[uv + 2] & 0xff) - offset) * force
                  : 0),
            ]);
          });
          return self;
    ...
    return a;
      ...
      MOD3.Perlin = MOD3.Class(MOD3.Modifier, {
        constructor: function Perlin(force, noise, autoRun) {
          var self = this;
          if (!(self instanceof Perlin))
            return new Perlin(force, noise, autoRun);
          self.$super('constructor');
          self.name = 'Perlin';
        ...
        apply: function (modifiable) {
          ...
          MOD3.List.each(modifiable.vertices, function (v) {
            ...
            v.setXYZ([
              xyz[0] +
                (axes & MOD3.ModConstant.X ? (pn[uv] - offset) * force : 0),
              xyz[1] +
                (axes & MOD3.ModConstant.Y
                  ? (pn[uv /*+1*/] - offset) * force
                  : 0),
              xyz[2] +
                (axes & MOD3.ModConstant.Z
                  ? (pn[uv /*+2*/] - offset) * force
                  : 0),
            ]);
          });
          return self;
